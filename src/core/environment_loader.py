...
def load_config_with_env(config_dir: Path, environment: str = "development") -> Dict[str, Any]:
    """
    ...
    if base_config_path.exists():
        try:
            base_config = loader.load_yaml_with_env(base_config_path)
            config_data.update(base_config)
        except Exception as e:
            print(f"Warning: Could not load base config: {e}")
    
    # Load environment-specific configuration
    env_config_path = config_dir / f"{environment}_config.yaml"
